...
class phpbb_ext_phpbb_karma_includes_manager
{
	 ...
	 * @return	array					An array of arrays with received karma generated by convert_karma_row_to_usable_values
	 */
	public function get_karma_received_by_user($user_id, $karma_per_page, $start_at, $newestfirst = true)
	{
		...
		{
			$received_karma[] = $this->convert_karma_row_to_usable_values($row);
		}
		$this->db->sql_freeresult($result);
	 ...
	 * @return	array|bool		An array with the information generated by convert_karma_row_to_usable_values; false otherwise
	 */
	public function get_karma_data($karma_id)
	{
		$sql_array = array(
			'SELECT'	=> 'k.*, kt.karma_type_name, u.user_id, u.username, u.user_colour',
			'FROM'		=> array(
				$this->karma_table			=> 'k',
				$this->karma_types_table	=> 'kt',
				USERS_TABLE					=> 'u',
		...
		return ($karma_row !== false) ? $this->convert_karma_row_to_usable_values($karma_row) : false;
	}
	...
	// TODO PHPDoc
	private function convert_karma_row_to_usable_values($karma_row) {
